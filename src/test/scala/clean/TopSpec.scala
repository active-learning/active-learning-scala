...
class TopSpec extends UnitSpec with Blob with Lock {
  if (false) {
    lazy val datasets = Source.fromFile("p2-spambase-wine5.txt").getLines().toList.filter(!_.startsWith("#")).takeWhile(!_.startsWith("!")).par
    val path = "/run/shm/testuci-lixo"
    ...
    datasets foreach { dataset =>
      ...
      if (!ds.isQCalculated) {
        Datasets.kfoldCV(set, 2) { (tr, ts, fold, minSize) =>
          ...
          //queries
          ds.writeQueries(RandomSampling(pool), run, fold, Int.MaxValue)
          ...
          learners foreach { learner =>
            ds.writeHits(pool.size, testSet, dsQueries.toVector, RandomSampling(pool), run, fold)(learner)
