...
\end{table}
  ...
  def extensiveTable2(capti: String, take11: Boolean, precision: Double, measures: Seq[(String, Seq[(Double, Double)])], strategies: Vector[String], tableName: String, measure: String, seps: Int = 4, language: String = "pt", sohcore: Boolean = false) = {
    val nnn = 14
    ...
    if (core.nonEmpty) {
      if (sohcore) core else table(capti, core, nstrats, if (take11) strategies.take(nnn) else strategies.drop(nnn), tableName, measure, language)
    } else ""
  }
