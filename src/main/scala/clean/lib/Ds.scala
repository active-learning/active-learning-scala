...
case class Ds(dataset: String, readOnly: Boolean) extends Db(s"$dataset", readOnly) with Blob with CM with EntropyMeasure with FilterTrait with LearnerTrait {
   ...
   lazy val metaAttsHumanAndKnowingLabels = List[Double](nclasses, nattributes, poolSize, poolSizeByNatts, 100d * nomCount / nattributes, majority, minority, majority / minority, normalized_entropy(hist))
   ...
   def areHitsFinished(poolSize: Int, testSet: Seq[Pattern], strat: Strategy, learner: Learner, run: Int, fold: Int, binaf: Filter, zscof: Filter, completeIt: Boolean, expectedAmount0: Int) = if (readOnly) error("read only")
      ...
      if (learner.id != strat.learner.id && strat.id > 1 && !Global.agnosticas.contains(strat.id)) error(s"areHitsFinished: Provided learner $learner is different from gnostic strategy's learner $strat.${strat.learner}")
   ...
   def writeHits(poolSize: Int, testSet: Seq[Pattern], queries: Vector[Pattern], strat: Strategy, run: Int, fold: Int, h: Int)(learner: Learner) = if (readOnly) error("read only")
   else if (learner.id != strat.learner.id && strat.id > 1 && !Global.agnosticas.contains(strat.id))
      ...
      val insertIntoP = poolId(strat, learner, run, fold) match {
         case Some(pid) => if (strat.id < 2 || Global.agnosticas.contains(strat.id)) quit(s"Pool $run.$fold já estava gravado para $strat.$learner referente aos hits de $strat.") else "SELECT 1"
         case None =>
