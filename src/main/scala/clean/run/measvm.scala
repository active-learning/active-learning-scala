...
object measvm extends Exp with LearnerTrait with StratsTrait with Lock with CM with RangeGenerator {
   ...
   def op(ds: Ds, pool: Seq[Pattern], testSet: Seq[Pattern], fpool: Seq[Pattern], ftestSet: Seq[Pattern], learnerSeed: Int, run: Int, fold: Int, binaf: Filter, zscof: Filter) {
      ...
      if (passivas) {
         ...
         for (strat <- allStrats(); learner <- Seq(SVMLibRBF(learnerSeed)); (ti, tf) <- Seq((tmin, thalf), (thalf, tmax), (tmin, tmax), (tmin, 49))) {
            strat match {
         ...
         for (strat <- allStrats(); learner <- Seq(SVMLibRBF(learnerSeed)); t <- tmin to tmax) {
            strat match {
               case Majoritary(Seq(), false) => //| SVMmulti(Seq(), "KFFw", false) | SVMmulti(Seq(), "BALANCED_EEw", false) => //jah foi acima
               case s =>
         ...
         for (strat <- allStrats(); learner <- Seq(SVMLibRBF(learnerSeed))) {
            val t = tpass
            strat match {
               case Majoritary(Seq(), false) => // | SVMmulti(Seq(), "KFFw", false) | SVMmulti(Seq(), "BALANCED_EEw", false) => //jah foi acima
               case s =>
   ...
   def datasetFinished(ds: Ds) {
      if (acabou) {
         ds.markAsFinishedMea(passivas + allStrats().map(_.limpa).mkString + Seq(SVMLibRBF()).map(_.limpa).mkString)
   ...
   def isAlreadyDone(ds: Ds) = {
      acabou = false
      ds.isFinishedMea(passivas + allStrats().map(_.limpa).mkString + Seq(SVMLibRBF()).map(_.limpa).mkString)
   }
