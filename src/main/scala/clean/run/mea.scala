...
object mea extends Exp with LearnerTrait with StratsTrait with Lock with CM with MeasuresTrait {
  ...
  def op(ds: Ds, pool: Seq[Pattern], testSet: Seq[Pattern], fpool: Seq[Pattern], ftestSet: Seq[Pattern], learnerSeed: Int, run: Int, fold: Int, binaf: Filter, zscof: Filter) {
    allMeasures(maxtimesteps) foreach { meas => if (!sqls.contains("cancel")) {
      if (meas.id == 15 || meas.id == 16) {
        ...
        stratsemLearnerExterno() foreach { strat =>
          if (!ds.areQueriesFinished(pool.size, strat, run, fold, null, null, completeIt = false, maxtimesteps)) {
  ...
  def storeSQL(poolSize: Int, ds: Ds, strat: Strategy, run: Int, fold: Int, testSetSize: Int, meas: Measure)(learner: Learner): Unit = {
    ...
    if (!ds.areHitsFinished(poolSize, Seq(), strat, learner, run, fold, null, null, completeIt = false, maxtimesteps - ds.nclasses + 1)) {
      ...
      case None =>
        val cms = ds.getCMs(strat, learner, run, fold, maxtimesteps - ds.nclasses + 1)
        if (cms.size < maxtimesteps - ds.nclasses + 1) ds.quit(s"Couldn't take at least ${maxtimesteps - ds.nclasses + 1} queries, ${cms.size} only.")
