      ...
      if (passivas) {
         ...
         for (strat <- allStrats(); learner <- allLearners(); (ti, tf) <- Seq((tmin, thalf), (thalf, tmax), (tmin, tmax), (tmin, 49))) {
            strat match {
               ...
               case s =>
                  if (!Global.stratsSemLExt.contains(strat.id) || (strat.id == 1006600 && learner.id == 11) || (strat.id == 292212 && learner.id == 773) || (Seq(966000, 967000, 968000, 969000).contains(strat.id) && Seq(165111, 556665).contains(learner.id)) || (Seq(966009, 967009, 968009, 969009).contains(strat.id) && learner.id == 2651110)) {
         ...
         for (strat <- allStrats(); learner <- allLearners(); t <- tmin to tmax) {
            strat match {
               case Majoritary(Seq(), false) => //| SVMmulti(Seq(), "KFFw", false) | SVMmulti(Seq(), "BALANCED_EEw", false) => //jah foi acima
               case s =>
                  if (!Global.stratsSemLExt.contains(strat.id) || (strat.id == 1006600 && learner.id == 11) || (strat.id == 292212 && learner.id == 773) || (Seq(966000, 967000, 968000, 969000).contains(strat.id) && Seq(165111, 556665).contains(learner.id)) || (Seq(966009, 967009, 968009, 969009).contains(strat.id) && learner.id == 2651110))  poeNaFila(fila, Kappa(ds, s, learner, run, fold)(t).sqlToWrite(ds))
            }
         }
         for (strat <- allStrats(); learner <- allLearners()) {
            ...
            strat match {
               case Majoritary(Seq(), false) => // | SVMmulti(Seq(), "KFFw", false) | SVMmulti(Seq(), "BALANCED_EEw", false) => //jah foi acima
               case s =>
                  if (!Global.stratsSemLExt.contains(strat.id) || (strat.id == 1006600 && learner.id == 11) || (strat.id == 292212 && learner.id == 773) || (Seq(966000, 967000, 968000, 969000).contains(strat.id) && Seq(165111, 556665).contains(learner.id)) || (Seq(966009, 967009, 968009, 969009).contains(strat.id) && learner.id == 2651110))  poeNaFila(fila, BalancedAcc(ds, s, learner, run, fold)(t).sqlToWrite(ds))
            }
         }
         if (fila.exists(_.startsWith("insert"))) ds.batchWrite(fila.toList)
         fila.clear()
