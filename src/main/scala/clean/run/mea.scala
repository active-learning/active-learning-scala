...
object mea extends Exp with LearnerTrait with StratsTrait with Lock with CM with RangeGenerator {
   ...
   def poeNaFila(fila: mutable.Set[String], str: String): Unit = try {
      fila += str
   } catch {
      case e: Throwable =>
   }
   ...
   def op(ds: Ds, pool: Seq[Pattern], testSet: Seq[Pattern], fpool: Seq[Pattern], ftestSet: Seq[Pattern], learnerSeed: Int, run: Int, fold: Int, binaf: Filter, zscof: Filter) {
      ...
      for (strat <- allStrats(); learner <- allLearners(); t <- i to f) {
         strat match {
            case Majoritary(Seq(), false) | SVMmulti(Seq(), "KFFw", false) | SVMmulti(Seq(), "BALANCED_EEw", false) => //jah foi acima
            case s =>
               poeNaFila(fila, Kappa(ds, s, learner, run, fold)(i).sqlToWrite(ds))
               poeNaFila(fila, BalancedAcc(ds, s, learner, run, fold)(f).sqlToWrite(ds))
         }
      }
