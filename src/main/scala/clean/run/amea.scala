...
object amea extends Exp with LearnerTrait with StratsTrait with RangeGenerator {
   ...
   def datasetFinished(ds: Ds) = {
      if (acabou && !outroProcessoVaiTerminarEsteDataset) {
         ds.markAsFinishedRun("amea6" + (stratsFpool().map(_(NoLearner())) ++ stratsPool().map(_(NoLearner())) ++ allLearners()).map(x => x.limpa).mkString)
   ...
   def isAlreadyDone(ds: Ds) = ds.isFinishedRun("amea6" + (stratsFpool().map(_(NoLearner())) ++ stratsPool().map(_(NoLearner())) ++ allLearners()).map(x => x.limpa).mkString)
