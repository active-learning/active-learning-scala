...
object metaEscolhePar extends AppWithUsage with LearnerTrait with StratsTrait with RangeGenerator with Rank with MetaTrait {
  ...
  override def run() = {
    ...
    pares1 foreach { case (estr, apren) =>
      ...
      val arq = s"/home/davi/wcs/arff/$context-$porPool-n${if (porRank) 1 else n}best${criterio}m$measure-$ini.$fim-${parName + (if (porRank) "Rank" else "")}-${txts.mkString.replace("-","")}-U$dsminSize.arff"
      val labels = txts
      val labelssts = txts
