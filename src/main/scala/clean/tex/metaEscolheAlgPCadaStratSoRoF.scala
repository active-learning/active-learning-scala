...
import clean.lib._
import clean.tex.metaEscolheAlgPCadaStrat._
import ml.Pattern
import ml.classifiers._
import util.{Datasets, Tempo}

object metaEscolheAlgPCadaStratSoRoF extends AppWithUsage with LearnerTrait with StratsTrait with RangeGenerator with Rank with MetaTrait {
    ...
    stratsTexForGraficoComplexo foreach { strat =>
      ...
      metads.readString(sql69) match {
        ...
        case x: List[Vector[String]] if x.isEmpty | guardaSohRank | true=>
          val cvs = cv(porPool, ini, fim, labelsleas, stratName, ntrees, patterns, metaclassifs, porRank, rus, ks, guardaSohRank, paralela = true, fake = false, sohRoF = true).toVector
