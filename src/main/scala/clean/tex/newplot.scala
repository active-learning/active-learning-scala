...
object newplot extends AppWithUsage with LearnerTrait with StratsTrait with RangeGenerator with Rank {
   ...
   override def run() = {
      super.run()
      val arq = s"/home/davi/wcs/tese/accbal$pioresAignorar$dist$tipoSumariz$tipoLearner" + (if (porRank) "Rank" else "") + (if (porRisco) "Risco" else "") + ".plot"
      println(s"$arq")
      ...
      val tests = for {
         ...
         val (stras, curvas) = (for {
            ...
            val v25 = for {
               r <- 0 until runs
               f <- 0 until folds
            } yield BalancedAcc(ds, s, le, r, f)(0).readAll(ds).getOrElse(throw new Error(s"NA: ${(ds, s, le.abr)}"))
