...
object tableMeta extends App {
  ...
  val sortedMCs = List("PCT+ ELM", "ELM", "PCT", "RFw", "C4.5w", "5NN", "maj") //maj tem que ser o último!
  ...
  db.open()
  val dbrows = db.readString("select st,mc,abts,dbts,htr from r where rs=10 AND fs=10 and nt=1000 and (sm like 'nosm%') and mc in (" + sortedMCs.map {
    case "PCT+ ELM" => "'PEr-a'"
    case "PCT" => "'PCTr-a'"
    case "ELM" => "'ELMr-a'"
    case "RFw" => "'RFw1000'"
  ...
  val tableheader = "\t& " + (sortedMCs ++ List("$\\delta$")).mkString(" & ") + " \\\\ \\hline"
  val tablerows0 = dbrows.groupBy(_.head).values.toList.sortBy(_.head.head).map { case veclst =>
    val strat = veclst.head.head
    val Min = (veclst map { case Vector(st, metacla, acc, dev, _) => acc }).dropRight(1).min //maj nao interessa
    ...
    }) ++ List(s"${"%2d".format(((Min.toDouble - ord2(veclst).head.reverse.tail.head.toDouble) * 100).round)}")
  ...
  val tablerows = tablerows0.sortBy(_.split("&")(sortedMCs.size + 1).split("\\\\\\\\").head.toDouble).reverse.mkString(" \\\\ \n")
  println(s"\\begin{table}[h]\n\\begin{center}\n\\begin{tabular}{l|b{1cm}" + Seq.fill(tableheader.split("&").size - 2)("r").mkString + "}")
