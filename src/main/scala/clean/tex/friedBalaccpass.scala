   ...
   override def run() = {
      ...
      for (le <- learners(learnersStr).par) {
         ...
         sorted.grouped(32).zipWithIndex.foreach { case (res1, i) =>
            fw.write(StatTests.extensiveTable2(true, 100, res1.toSeq.map(x => x._1 -> x._2), sl.toVector.map(_.toString), s"stratsBalAcc${i}a" + le.abr, "acurácia balanceada para " + le.abr, 7))
            fw.write(StatTests.extensiveTable2(false, 100, res1.toSeq.map(x => x._1 -> x._2), sl.toVector.map(_.toString), s"stratsBalAcc${i}b" + le.abr, "acurácia balanceada para " + le.abr, 7))
         }
         fw.close()
