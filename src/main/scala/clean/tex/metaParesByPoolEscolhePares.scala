...
object metaParesByPoolEscolhePares extends AppWithUsage with LearnerTrait with StratsTrait with RangeGenerator with Rank with MetaTrait {
  ...
  override def run() = {
    ...
    val pares = for {strat <- stratsMinimal("eucl"); l <- ls} yield strat -> l
    val spares = pares.map { case (s, l) => s(l).limpa }.mkString("-")
    Tempo.start
    //    $rus.$ks
    val arq = s"/home/davi/wcs/arff/$semR${if (suav) "suav" else ""}$context-n${if (porRank) 1 else n}best${criterio}m$measure-$ini.$fim-${"pares" + (if (porRank) "Rank" else "")}-$spares-p$dsminSize$featureSel.arff"
    ...
    if (porRank) print(s"${"pares"} Spearman correl. $rus*$ks-fold CV. ${if (smote) "-SMOTE" else ""}" + " " + arq + " ")
    else print(s"${"pares"} Accuracy. $rus*$ks-fold CV. $featureSel ${if (smote) "-SMOTE" else ""}" + " " + arq + " ")
    ...
    metads.open()
    metads.readString(s"select mc from r where ls='$spares' and st='${"pares"}' and sm='$sm' and nt=$ntrees and fsel='$fsel' and ra='$ra' and rs=$rus and fs=$ks") match {
      case x: List[Vector[String]] if x.map(_.head).intersect(metaclassifs(Vector()).map(_.limp)).size == 0 =>
        ...
        porMetaLea foreach { case (nome, resultados) =>
          ...
          metads.write(s"insert  into r values ('$ra', '$sm', $criterio, '$ini', '$fim', '${"pares"}', '$spares', $rus, $ks, '$nome', $ntrees, '$fsel', $dsminSize, ${accTr._1}, ${accTr._2}, ${accTs._1}, ${accTs._2}, ${accBalTr._1}, ${accBalTr._2}, ${accBalTs._1}, ${accBalTs._2}, '$resumoTr', '$resumoTs')")
