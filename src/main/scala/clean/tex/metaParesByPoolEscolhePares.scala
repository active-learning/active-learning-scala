...
object metaParesByPoolEscolhePares extends AppWithUsage with LearnerTrait with StratsTrait with RangeGenerator with Rank with MetaTrait {
  ...
  override def run() = {
    ...
    pares foreach { case (p0, l0) =>
      ...
      if (porRank) print(s"$parName Spearman correl. $rus*$ks-fold CV. ${if (smote) "-SMOTE" else ""}" + " " + arq + " ")
      else print(s"$parName Accuracy. $rus*$ks-fold CV. $featureSel ${if (smote) "-SMOTE" else ""}" + " " + arq + " ")
      ...
      metads.open()
      metads.readString(s"select mc from r where ls='$spares' and st='$parName' and sm='$sm' and nt=$ntrees and fsel='$fsel' and ra='$ra' and rs=$rus and fs=$ks") match {
        case x: List[Vector[String]] if x.map(_.head).intersect(metaclassifs(Vector()).map(_.limp)).size == 0 =>
          val porMetaLea = cv(parName, smotePropor, smote, ntrees, featureSel, patterns, metaclassifs, porRank, rus, ks).toVector.flatten.flatten.groupBy(_.metalearner)
          ...
          porMetaLea foreach { case (nome, resultados) =>
            ...
            metads.write(s"insert  into r values ('$ra', '$sm', $criterio, '$ini', '$fim', '$parName', '$spares', $rus, $ks, '$nome', $ntrees, '$fsel', $dsminSize, ${accTr._1}, ${accTr._2}, ${accTs._1}, ${accTs._2}, ${accBalTr._1}, ${accBalTr._2}, ${accBalTs._1}, ${accBalTs._2}, '$resumoTr', '$resumoTs')")
