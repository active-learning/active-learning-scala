...
trait MetaTrait extends FilterTrait with Rank with Log {
  ...
  def cv(porPool: Boolean, ti: String, tf: String, labels: Seq[String], strat: String, ntrees: Int, patterns: Vector[Pattern], leas: Vector[Pattern] => Vector[Learner], rank: Boolean, rs: Int, ks: Int, readOnly: Boolean = false, paralela: Boolean = true, fake: Boolean = false, sohRoF: Boolean = false) = {
    ...
    val rrr = (if (paralela) (0 to rs - 1).par else 0 to rs - 1).map { run =>
      ...
      val shuffled = if (sohRoF || fake) new Random(run).shuffle(bagsrefeito.sortBy(x => (x.head.toString + x.head.nomeBase).hashCode)) else new Random(run).shuffle(bagsrefeito)
            ...
            Vector(tstest -> (for (a <- 0 to 4444; b <- 0 to 4) yield a -> b)) foreach { case (tx, rfs) =>
              ...
              (tx, rfs).zipped.toList.zipWithIndex.foreach { case ((pat, (r, f)), idx) =>
                ...
                val sql = if (!porPool) {
                  ...
                  else if (this.getClass.getName.contains("Dist")) s"insert into tenfold values ('$base', '$ti', '$tf', '$strat', '$labels', '${mc.limp}', '${esperado.split("-").head}', '${predito.split("-").head}', $run, $fold, $idx)"
                  else if (this.getClass.getName.contains("AA")) s"insert into tenfold values ('$base', '$ti', '$tf', '$strat', '$labels', '${mc.limp}', '${esperado.split("-").head}', '${predito.split("-").head}', $run, $fold, $idx)"
                  ...
                  s"insert into acc values ('${if (trtest == tx) "tr" else "ts"}', '$base', '$ti', '$tf', '$strat', '$labels', '${mc.limp}', '${esperado.split("-").last}', '${predito.split("-").last}', '$r', '$f')"
                }
                sqls += sql
              }
            }
