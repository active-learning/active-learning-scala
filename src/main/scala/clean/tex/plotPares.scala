...
object plotPares extends AppWithUsage with LearnerTrait with StratsTrait with RangeGenerator with Rank {
  lazy val arguments = superArguments ++ List("learners:nb,5nn,c45,vfdt,ci,...|eci|i|ei|in|svm", "porRank:r", "porRisco:r", "dist:euc,man,mah", "sóRnd:rnd", "só4sts:4")
  ...
  val strats = if (args(9) == "4") stratsFim
  else if (sohRnd) Seq((l: Learner) => RandomSampling(Seq()))
