    ...
    les foreach { lea =>
      ...
      metads.readString(sql69) match {
        ...
        case x: List[Vector[String]] if x.isEmpty =>
          val porMetaLea = cv(porPool, ini, fim, labelssts, leaName, ntrees, patterns, metaclassifs, porRank, rus, ks).toVector.flatten.flatten.groupBy(_.metalearner)
