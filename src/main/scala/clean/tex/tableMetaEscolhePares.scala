...
object tableMetaEscolhePares extends App {
  ...
  db.open()
  val sqla = "select st,mc,abts,dbts,htr from r where rs=10 AND fs=10 and nt=1000 and (sm like 'nosm%') and ls='Rnd.Clu.ATUeuc.HTUeuc.Mar.TUeuc.EERent--5NN.RFw.SVM' and st like '%-%-%' and mc in (" + sortedMCs.map {
    case "ELM+\\newline PCT" => "'PE'"
    case "PCT" => "'PCT'"
    case "ELM" => "'ELM'"
    case "RFw" => "'RFw1000'"
  ...
  val sqlr = "select st,mc,ats,dts,htr from r where rs=10 AND fs=10 and nt=1000 and (sm like 'nosm%') and ls='Rnd.Clu.ATUeuc.HTUeuc.Mar.TUeuc.EERent--5NN.RFw.SVM' and st like '%-%-%' and mc in (" + sortedMCs.map {
    case "PCT+ ELM" => "'PEr'"
    case "PCT" => "'PCTr'"
    case "ELM" => "'ELMr'"
    case "def" => "'defr'"
