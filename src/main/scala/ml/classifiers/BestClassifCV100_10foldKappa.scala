...
case class BestClassifCV100_10foldKappa(ds: Ds, r: Int, f: Int, s: Strategy, queries: Seq[Pattern], fqueries: Seq[Pattern], seed: Int, poolForKNN: Seq[Pattern]) extends Learner with CM {
   override lazy val toString = s"BestClassifCV100_10foldk: $ds"
   lazy val id = ds.read(s"select c from classif10010foldk where s=${s.id} and l=${s.learner.id} and r=$r and f=$f") match {
      case List(Vector(x)) => x.toInt
      case List() =>
         val res = classif.id
         ds.write(s"insert into classif10010foldk values (${s.id},${s.learner.id},$r,$f,$res)")
         res
   ...
   val context: String = "bestcv100-10foldk"
}
