...
trait AgStrategyWithLearnerAndMapsLU extends Strategy with DistanceMeasure {
   ...
   protected def resume_queries_impl(unlabeled: Seq[Pattern], labeled: Seq[Pattern]) = {
      ...
      queries_rec(initial_mapU, initial_mapsL.toSeq, unlabeled, labeled, hist)
   }

   private def queries_rec(mapU: => Map[Pattern, Double], mapsL: => Seq[Map[Pattern, Double]], unlabeled: Seq[Pattern], labeled: Seq[Pattern], hist: Seq[Int]): Stream[Pattern] = {
      if (unlabeled.isEmpty) Stream.Empty
      else {
         if (debug) visual_test(null, unlabeled, labeled)
         val selected = next(mapU, mapsL, unlabeled, labeled, hist)
         ...
         lazy val newsL = mapsL map { mapL =>
            if (mapL.contains(selected)) (mapL - selected) transform { case (pa, si) => si + 1d / (1 + d(selected, pa))}
            else mapL
         }

         if (debug) visual_test(selected, unlabeled, labeled)
         selected #:: queries_rec(newU, newsL, unlabeled.diff(Seq(selected)), labeled :+ selected, newHist)
      }
   }
