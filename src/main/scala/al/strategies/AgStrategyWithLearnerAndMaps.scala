...
trait AgStrategyWithLearnerAndMaps extends Strategy with DistanceMeasure {
   ...
   protected def resume_queries_impl(unlabeled: Seq[Pattern], labeled: Seq[Pattern]) = {
      ...
      queries_rec(initial_mapU, initial_mapL, unlabeled, labeled)
   }

   private def queries_rec(mapU: => Map[Pattern, Double], mapL: => Map[Pattern, Double], unlabeled: Seq[Pattern], labeled: Seq[Pattern]): Stream[Pattern] = {
      if (unlabeled.isEmpty) Stream.Empty
      else {
         if (debug) visual_test(null, unlabeled, labeled)
         val selected = next(mapU, mapL, unlabeled, labeled)
         ...
         selected #:: queries_rec(newU, newL, unlabeled.diff(Seq(selected)), labeled :+ selected)
      }
   }
