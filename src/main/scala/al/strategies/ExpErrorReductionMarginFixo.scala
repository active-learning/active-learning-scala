 ...
 * @param debug
 */
case class ExpErrorReductionMarginFixo(learner: Learner, pool: Seq[Pattern], criterion: String, sample: Int = 100, debug: Boolean = false)
   extends StrategyWithLearner with Sample with EntropyMeasure {
   override val toString = "Expected Error Reduction Margin s" + learner.limpa + sample + " (" + criterion + ")"
   val abr = "EER" + learner.limpa + (if (criterion == "balacc") "acu" else criterion.take(3))
         ...
         val (selected, label_estimate, _) = (
            for (pattern <- unlabeledSamp; c <- 0 until nclasses) yield {
               ...
               criterionInt match {
                  ...
                  case Balacc => (pattern, c, 1 - criterion_balacc(art_model, optimistic_patterns))
   ...
   private def criterion_balacc(m: Model, label_estimated_patterns: Vector[Pattern]) = {
      val pseudo_accuracies_per_class = (0 until nclasses) map {
         c =>
            val only_this_class = label_estimated_patterns.filter(_.label == c)
            val hits = only_this_class count m.hit
      ...
      pseudo_accuracies_per_class.sum / nclasses
   }
