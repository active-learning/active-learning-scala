...
trait StrategySG extends Strategy {
  ...
  protected def resume_queries_impl(unlabeled: Seq[Pattern], labeled: Seq[Pattern]): Stream[Pattern] = {
    val specificPoolsPlusLabeled = {
      for (c <- 0 until nclasses) yield (labeled ++ (unlabeled map (_.relabeled_reweighted(c, background_weight, new_missed = false)))).toList
    }.toArray
