...
object ALELMExample extends App {
   val patts0 = new Random(0).shuffle(Datasets.arff("/home/davi/wcs/ucipp/uci/banana.arff").right.get.take(2000))
   ...
   val tris = DensityWeightedTrainingUtility(IELMBatch(), patts.take(n), "eucl").queries
   ...
   val treci = DensityWeightedTrainingUtility(CIELMBatch(), patts.take(n), "eucl").queries
   val ts = patts.drop(n)
