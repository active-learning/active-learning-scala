...
object SamePatterns extends App {
  //testar só ids (cenário real) ou realizar bina?
  val testarSohIdELabel = false //true //false
  ...
  val res = l forall { d =>
    ...
    // the data on SQLite doesn't have header info anymore and register only the internal label from 0 until |Y|.
    val b = Datasets.arff(!testarSohIdELabel)(db.path + db.database + ".arff", zscored = false, preserveClassOrderFromARFFHeader = false).right.get

    //compara
    val sa = if (testarSohIdELabel) a.map(p => List(p.id, p.label)).sameElements(b.map(p => List(p.id, p.label))) else a.sameElements(b)
    println(s"d: " + sa)
    //    if (!sa) {
    a.toList take 3 foreach (x => println(x.toStringCerto))
    println("-----")
    b.toList take 3 foreach (x => println(x.toStringCerto))
    println("")
    //    }
    sa
  }
