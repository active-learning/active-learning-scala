...
object SamePatterns extends App {
  ...
  val res = l forall { d =>
    ...
    val b = Datasets.arff(bina = !testarSohIdELabel)(db.path + db.database + ".arff", zscored = false, preserveClassOrderFromARFFHeader = false).right.get
    ...
    if (!sa) {
      ...
      println("")
      println(a.toList.diff(b.toList).map(_.toStringCerto))
      println("")
      println(b.toList.diff(a.toList).map(_.toStringCerto))
    }
    sa
  }
