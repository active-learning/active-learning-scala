...
object SameRndQueriesTest extends App {
  ...
  val res = l forall { d =>
    ...
    //simula CV
    val run = 3
    val patts = Lazy(new Random(run).shuffle(Datasets.arff(false)("/home/davi/wcs/ucipp/uci/" + d + ".arff", preserveClassOrderFromARFFHeader = false).right.get))
    ...
    da.saveQueries(RandomSampling(pool), run, fold, if (asd) f else null, 234234, m)
    da.save()

    //carrega queries do dataset que era vazio via ARFF e via SQLite
    val aarff = ALDatasets.queriesFromMap(da.path + da.database + ".arff")(da)(RandomSampling(Seq()), run, fold, m).right.get
    val asqlite = ALDatasets.queriesFromSQLite(da)(RandomSampling(Seq()), run, fold).right.get
    da.close()

    //carrega queries do dataset que já as tinha via SQLite
    val bsqlite = ALDatasets.queriesFromSQLite(db)(RandomSampling(Seq()), run, fold).right.get
    db.close()

    val basq = bsqlite.sameElements(asqlite)
    val baar = bsqlite.sameElements(aarff)
    println(s"$basq $baar")
